...
def rtkp_pos(parsed_args: argparse.Namespace, logger: Logger) -> pl.DataFrame:
    ...
    qual_analysis, qual_tabular = quality_analysis(
        df_pnt=pos_df, file_name=os.path.basename(parsed_args.pos_ifn), logger=logger
    )

    # print quality in tabular form
    rprint(
        f"\nAnalysis of the quality of the position data for\
        {os.path.basename(parsed_args.pos_ifn)}:\n{qual_tabular}"
    )
