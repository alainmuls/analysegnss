...
def gpsms2dt(week: int,
             towms: float) -> datetime:
    """
    :param week: GPS week number, i.e. 1866
    ...
    if isnan(week) or isnan(towms) or towms == 4294967295:
        ...
        date_before_leaps = _GPS_EPOCH + timedelta(seconds=week * _SECS_IN_WEEK + towms / 1000)
        return date_before_leaps


def gnss2utc(week: int,
             tow: float) -> datetime:
    """
    :param week: GPS week number, i.e. 1866
    :param secs: number of seconds since the beginning of `week`
...
def utc2gnss(utc: str,
             utc_parser: str) -> datetime:
    """
...
def main(argv: list) -> None:
    ...
    print(f"utc2gnssutc=()'2020-01-10 07:59:42', utc_parser='%Y-%m-%d %H:%M:%S')   = {utc2gnss(utc='2020-01-10 07:59:42', utc_parser='%Y-%m-%d %H:%M:%S')}")
    print(f"utc2gnss(utc='23032022-14:52:23.012', utc_parser='%d%m%Y-%H:%M:%S.%f') = {utc2gnss(utc='23032022-14:52:23.012', utc_parser='%d%m%Y-%H:%M:%S.%f')}")
