...
from analysegnss.scripts.ebh.ebh_print import print_ebh_ok, print_ebh_nok, print_starting_ppk_process
...
def ebh_process_launcher(parsed_args: argparse.Namespace, logger: Logger) -> None:
    ...
    try:
        ...
        rtk_result = process_rtk_solution(parsed_args=parsed_args, ebh_timings=ebh_timings, logger=logger)
        ...
        ppk_result = process_ppk_solution(parsed_args=parsed_args, rtk_result=rtk_result, ebh_timings=ebh_timings, logger=logger)
        if ppk_result.get("success", False):
            ...
            try:
                logger.warning("Trying to calculate runway gradients using insufficient RTK/PPK solution.")
                rprint(f"\nTrying to calculate runway gradients using [yellow]insufficient RTK/PPK solution[/yellow]")
                calculate_runway_gradients(parsed_args=parsed_args, logger=logger)
...
def process_rtk_solution(parsed_args: argparse.Namespace, ebh_timings: dict, logger: Logger) -> dict:
    ...
    try:
        ...
        if qual_decision == "ALL-EBH-OK":
            print_ebh_ok(logger=logger, source="RTK")
            return {"success": True}
        else:
            print_ebh_nok(logger=logger, rejected_lines=rejected_lines, rejection_level=EBH_REJECTION_LEVEL, source="RTK")
            return {
                "success": False,
...
def process_ppk_solution(parsed_args: argparse.Namespace, rtk_result: dict, ebh_timings: dict, logger: Logger) -> dict:
    ...
    if not parsed_args.base_corr:
        logger.warning("Base corrections are not available. Not possible to calculate PPK.")
        rprint(f"[yellow]Base correction are not available. Not possible to calculate PPK.[/yellow]")
    ...
    try:
        ...
        del parsed_args.sbf_ifn # delete sbf_ifn from parsed_args to avoid it being used in the ebh_lines function
        # ebh_lines function is called with pos_ifn as input
        ebh_qual_ppk = ebh_lines(parsed_args=parsed_args, logger=logger) #TODO maybe adapt mutually exclusive group to only allow sbf_ifn or pos_ifn?
        ...
        if qual_decision == "ALL-EBH-OK":
            print_ebh_ok(logger=logger, source="PPK")
            return {"success": True}
        else:
            print_ebh_nok(logger=logger, rejected_lines=rejected_lines, rejection_level=EBH_REJECTION_LEVEL, source="PPK")
            return {"success": False}
...
def do_ppk_by_decision(
    ...
    ppk_pos_ofn = ""
    parsed_args.nav = []  # parsed_args.nav is configured with nargs=+ option in argument_parser_rnx2rtkp_launcher. This option requires the argument to be a list of strings.
    ...
    match ebh_qual_decision:
        ...
        case "1-EBH-NOK":
            ...
            if parsed_args.base_corr is None:
                ...
                logger.debug(f"base correction file is imported correctly: {parsed_args.base_corr}")
                
                # getting obs, nav and base coordinates from sbf_ifn
                rnx_obs_fn, rnx_nav_fn, base_coord_X, base_coord_Y, base_coord_Z = get_base_data_for_PPK(parsed_args=parsed_args, logger=logger)
        ...
        case "ALL-EBH-NOK":
            ...
            parsed_args.obs = rnx_obs_fn
            parsed_args.nav.append(rnx_nav_fn) # parsed_args.nav is configured with nargs=+ option. This option requires the argument to be a list of strings.
...
def get_base_data_for_PPK(parsed_args: argparse.Namespace, logger: Logger) -> tuple[str, str, str, str, str]:
    """
    ...
    logger.info(f"Using base coordinates: {base_coord_X}, {base_coord_Y}, {base_coord_Z}")

    # check format of base correction file
    if parsed_args.sbf_ifn.endswith(
        ".sbf"
    ) or parsed_args.sbf_ifn.endswith("_"):
